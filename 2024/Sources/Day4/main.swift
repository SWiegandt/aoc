...
func diagonal(_ lines: [String]) -> Int {
    ...
    for (n, line) in lines.enumerated() {
        ...
        for (m, _) in line.enumerated() {
            ...
            if "\(chars[n][m])\(chars[n+1][m+1])\(chars[n+2][m+2])\(chars[n+3][m+3])".starts(with: /XMAS|SAMX/) {
                sum += 1
            }
        }
    }
...
func one(_ lines: [String]) -> Int {
    horizontal(lines).sum + vertical(lines).sum + diagonal(lines) + diagonal(lines.map { String($0.reversed()) })
}
...
func two(_ lines: [String]) -> Int {
    ...
    for (n, line) in lines.enumerated() {
        ...
        for (m, c) in line.enumerated() {
            ...
            if !"\(chars[n-1][m-1])\(chars[n][m])\(chars[n+1][m+1])".starts(with: /MAS|SAM/) { continue }
            if !"\(chars[n-1][m+1])\(chars[n][m])\(chars[n+1][m-1])".starts(with: /MAS|SAM/) { continue }
